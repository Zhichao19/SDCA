function [d_dec] = get_d_dec(k,preceding,temp,lanes,v,a,M)
    ...
    if (delta_M~=0)
        tau_1_dec = max(0,floor(delta_v_dec/delta_M));
        tau_2_dec = max(0,floor((v(temp(k),lanes)-a(temp(k),lanes))/M(temp(k),lanes)));
        ...
        ubound_1 = max(floor((v(temp(k),lanes) - a(temp(k),lanes)) / M(temp(k),lanes)));
        ubound_2 = max(floor((v(temp(preceding),lanes) - M(temp(k),lanes)) / M(temp(k),lanes)));
